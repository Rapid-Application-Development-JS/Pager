(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(function () {
            return (root.Pager = factory());
        });
    } else if (typeof module === 'object' && module.exports) {
        module.exports = (root.Pager = factory());
    } else {
        root.Pager = factory();
    }
}(this, function () {
    ...
    (function (scope) {
        ...
        if ("now" in scope.performance == false) {
    ...
    (function (scope) {
        ...
        for (x = 0; x < vendors.length && !scope.requestAnimationFrame; x += 1) {
            scope.requestAnimationFrame = scope[vendors[x] + 'RequestAnimationFrame'];
            scope.cancelAnimationFrame = scope[vendors[x] + 'CancelAnimationFrame']
            || scope[vendors[x] + 'CancelRequestAnimationFrame'];
        }

        if (!scope.requestAnimationFrame) {
            scope.requestAnimationFrame = function (callback) {
                currTime = scope.performance.now();
                timeToCall = Math.max(0, 16 - (currTime - lastTime));
                id = scope.setTimeout(function () {
